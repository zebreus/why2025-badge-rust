...
**Power rail**:
A named supply net in the badge power design. The current M2 power schematic explicitly names at least `+5V`, `+3.3V`, and `+1V2` rails.
_Avoid_: voltage, power supply

**Buck Converter**:
A switching regulator stage in the badge power design. The current M2 power schematic uses `TPS563202DRLR`, described there as a `4.3-V to 17-V Input, 3-A Synchronous Buck Converter in SOT563`, to derive lower-voltage rails from `VIN`.
_Avoid_: charger, battery
...
- The **WHY2025 badge** has one or more **Power rails**
- The **WHY2025 badge** has one or more **Buck Converters**
...
- A **Buck Converter** may generate one or more **Power rails**
...
- "power", named rails, and regulator parts can blur the overall power design with specific nets and conversion stages — resolved: use **Power rail** for named supply nets such as `+5V`, `+3.3V`, and `+1V2`, and **Buck Converter** for the regulator stage that converts `VIN` into lower rails.
