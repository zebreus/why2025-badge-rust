...
- The repository provides raw ABI references, Emulation, examples, docs, toolchain build/link scripts, and conformance harnesses, but not the canonical `std` backend source.
- `why2025-badge-sys` remains the firmware ABI reference crate for Apps and tests. `std` must not depend on it as a crate.
