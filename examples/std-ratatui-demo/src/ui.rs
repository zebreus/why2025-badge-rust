...
pub fn render(frame: &mut Frame, app: &mut App) {
    ...
    let tabs = app
        ...
        .titles
        .iter()
        .map(|title| text::Line::from(Span::styled(*title, Style::default().fg(Color::Green))))
        .collect::<Tabs>()
    ...
    let chunks = Layout::horizontal(constraints).split(area);
    {
            ...
            .split(chunks[0]);
        {
            ...
            let tasks: Vec<ListItem> = app
                ...
                .items
                .iter()
                .map(|item| ListItem::new(vec![text::Line::from(Span::raw(*item))]))
                .collect();
...
fn draw_second_tab(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let rows = app.servers.iter().map(|server| {
        ...
        Row::new(vec![server.name, server.location, server.status]).style(style)
    });
    let table = Table::new(
        rows,
        [
    ...
    let map = Canvas::default()
        ...
        .paint(|ctx| {
            ...
            for (index, server_one) in app.servers.iter().enumerate() {
                for server_two in &app.servers[index + 1..] {
                    ctx.draw(&canvas::Line {
                        x1: server_one.coords.1,
                        y1: server_one.coords.0,
                        y2: server_two.coords.0,
                        x2: server_two.coords.1,
                        color: Color::Yellow,
                    });
                }
            }
...
fn draw_third_tab(frame: &mut Frame, _app: &mut App, area: Rect) {
    ...
    let items: Vec<Row> = colors
        ...
        .map(|color| {
            let cells = vec![
                Cell::from(Span::raw(format!("{color:?}: "))),
                Cell::from(Span::styled("Foreground", Style::default().fg(*color))),
                Cell::from(Span::styled("Background", Style::default().bg(*color))),
            ];
            Row::new(cells)
        })
        .collect();
