...
///   immediately calls `load_cookies_from_file(curl, filename)`
...
/// - `Domain`, `Path`, `Max-Age`, `Secure`, `HttpOnly`, and `SameSite` attributes are parsed
...
///   `SameSite`; every stored cookie is always appended to the outgoing `Cookie` header
/// - `CURLOPT_COOKIEFILE` and `CURLOPT_COOKIEJAR` use a custom tab-separated text format, not the
///   Netscape/libcurl cookie jar format
/// - when loading from disk, `samesite` is parsed as `(atoi(field) != 0)`, so a saved `STRICT`
///   value of `2` comes back as `1`, which corresponds to `LAX`
///
...
/// - allocation failures for temporary header buffers are not checked before `memcpy()`
